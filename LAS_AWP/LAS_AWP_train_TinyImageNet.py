...
from TinyImageNet_models.preact_resnet import PreActResNet18
from utils import *
...
import argparse
import logging
from TinyImageNet_StrategyNet import *
ImageNet_MEAN = (0., 0., 0.)
ImageNet_STD = (1, 1, 1)
...
def get_args():
    ...
    parser.add_argument('--data-dir', default='../tiny-image-200', type=str)
    ...
    parser.add_argument('--attack_iters_types', type=int, nargs="*", default=range(8, 15))
...
def main():
    ...
    out_dir = os.path.join(out_dir, 'policy_optimizer_' + args.policy_optimizer)

    out_dir = os.path.join(out_dir, 'policy_model_lr_' + str(args.policy_model_lr))
    # out_dir=os.path.join(out_dir,'clip_grad_norm_'+str(args.clip_grad_norm))
    out_dir=os.path.join(out_dir,'epochs_'+str(args.epochs))
    out_dir = os.path.join(out_dir, 'a_' + str(args.a) + '_b_' + str(args.b) + '_c_' + str(args.c))
    out_dir = os.path.join(out_dir, 'R2_param_' + str(args.R2_param))
    out_dir = os.path.join(out_dir, 'R3_param_' + str(args.R3_param))
    ...
    out_dir = os.path.join(out_dir, 'interval_num_' + str(args.interval_num))
    #
    ...
    if args.policy_optimizer == 'SGD_without_momentum':
        ...
        policy_optimizer = optim.SGD([{'params': Strategy_model.parameters(), 'initial_lr': args.policy_model_lr}],
                                     lr=args.policy_model_lr, momentum=0.9, weight_decay=5e-4)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            lr,policy_lr = lr_schedule(epoch + (i + 1) / len(train_loader))

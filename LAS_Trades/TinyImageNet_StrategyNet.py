...
class ResNet_Strategy(nn.Module):
    def __init__(self, block, num_blocks,  args):
        ...
        self.Attack_method = nn.Linear(512*block.expansion*4, len(args.attack_types))  # 所用攻击方式的个数 1
        self.Attack_epsilon = nn.Linear(512*block.expansion*4, len(args.epsilon_types))  # 攻击强度 11
        self.Attack_iters = nn.Linear(512*block.expansion*4, len(args.attack_iters_types))  # 迭代次数 11
        self.Attack_step_size = nn.Linear(512*block.expansion*4, len(args.step_size_types))  # 步长
        self.Attack_beta_size = nn.Linear(512 * block.expansion*4, len(args.beta_types))  # 正则
    ...
    def forward(self, x):
        ...
        Attack_beta = self.Attack_beta_size(out)
        return Attack_method, Attack_epsilon, Attack_iters, Attack_step_size, Attack_beta

import argparse
def get_args():
    parser = argparse.ArgumentParser('RLAT')
    # target model
    parser.add_argument('--batch-size', default=128, type=int)
    parser.add_argument('--data-dir', default='../cifar-data', type=str)
    parser.add_argument('--out-dir', default='RLAT', type=str, help='Output directory')
...
def ResNet152_Strategy(args):
    return ResNet_Strategy(Bottleneck, [3, 8, 36, 3],args)
